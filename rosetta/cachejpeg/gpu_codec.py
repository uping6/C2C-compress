...
from homo_c2c_kv.codec.cachejpeg.quant_table import build_frequency_table, layer_group
...
class GPUCacheJPEGCodec:
    ...
    def _encode_layer_data(
        ...
        for component_name, tensor in (("key", key), ("value", value)):
            ...
            layer_data[f"{component_name}_body"] = _pack_quantized(
                quantized, cfg.entropy.representation, cfg.entropy.backend
            )
            ...
            del component_body, coefficients, table, quantized
    ...
    def encode_layer(
        ...
        layer, diagnostics = self._encode_layer_data(layer_index, num_layers, key, value, cfg)
    ...
    def encode(self, past_key_values, config: dict[str, Any]) -> EncodedPayload:
        ...
        for layer_index, (key, value) in enumerate(past_key_values):
            layer_data, layer_diag = self._encode_layer_data(
                layer_index, num_layers, key, value, cfg
            )
